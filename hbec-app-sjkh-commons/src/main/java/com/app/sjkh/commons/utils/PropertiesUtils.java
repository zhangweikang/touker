...
public class PropertiesUtils implements ApplicationContextAware {
    ...
    class TimesTask extends TimerTask {
        @Override
        public void run() {
            Set<String> keys = redisService.keys(null, Constants.REDIS_KEY_PLAMFORM + "*");
            if (keys.size() > 0) {
                for (String key : keys) {
                    String value = redisService.get(key);
                    propertiesMap.put(key.substring(Constants.REDIS_KEY_PLAMFORM.length(), key.length()), value);
                }
            }
        }
    }
