...
//		require.async(protocol,function(module){
			...
			// 访问接口来源标识，访问来源(默认PC) 0：pc， 2：安卓平板， 3：安卓手机，4：IOS 
			// PC 
			if(platform == "0")
			{
				param.op_way = param.op_source = 0;
			}
			// Android
			else if(platform == "1")
			{
				if(iBrowser.android){
					param.op_way = param.op_source = 3;  // Android 手机端
				}
				else{
					param.op_way = param.op_source = 2;  // Android 平板
				}
			}
			// IOS 
			else if(iBrowser.iPhone || iBrowser.iPad)
			{
				
				//param.op_way = param.op_source = 4;
				
				//start by 2014-12-06   15:36    修改接口来源标识
				
				param.op_way = param.op_source = 3;
