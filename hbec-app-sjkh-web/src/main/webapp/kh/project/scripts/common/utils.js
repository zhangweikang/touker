	...
	function getIp(){
        ...
        function toukerServerPluginCallback (param){
			service.hbAjax(param, function (data) {
				...
				appUtils.setSStorageInfo("ip", data.results.dataSet.ip); // 将 ip 保存到 sessionStorage 里面
				appUtils.setSStorageInfo("mac", data.results.dataSet.mac); // 将 mac 保存到 sessionStorage 里面
			});
		}*/
